...
export const PmPaymentForm: React.FC<PmPaymentFormProps> = ({
  projectId,
  payment,
  onSubmit,
  onCancel,
  ...
  const fetchUsers = async () => {
    try {
      // Get team members with role 'Project Manager'
      const teamsResponse = await api.get('/teams', { params: { role: 'Project Manager' } });
      ...
      // Fallback: get users with Project Manager role
      try {
        const response = await api.get('/users');
        const allUsers = Array.isArray(response.data) ? response.data : [];
        const pmUsers = allUsers.filter((u: any) => u.role === 'Project Manager');
        setUsers(pmUsers);
      } catch (e) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (payment) {
        await api.put(`/project-pm-payments/${payment.id}`, data);
      } else {
        await api.post('/project-pm-payments', data);
      }

      onSubmit();
    } catch (error: any) {
      console.error('Error saving PM payment:', error);
